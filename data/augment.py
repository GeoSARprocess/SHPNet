...
def opencv_M_to_pytorch_theta(M, src_size, dst_size):
    """
    将OpenCV格式的变换矩阵M转换为PyTorch Grid Sample格式的theta矩阵

    M: 3x3矩阵, src_pixel -> dst_pixel (前向变换)
    theta: (1, 2, 3) tensor, dst_norm -> src_norm (逆向采样变换)

    关键: grid_sample需要"逆向采样"，即从目标图坐标采样原图像素
    """
    src_w, src_h = src_size
    dst_w, dst_h = dst_size

    # 1. 求M的逆 (dst_pixel -> src_pixel)
    try:
        M_inv = np.linalg.inv(M)
    except:
        M_inv = np.linalg.pinv(M)

    # 2. 构建坐标转换矩阵 (align_corners=True)
    # dst_norm [-1,1] -> dst_pixel [0, dst_w-1]
    N_dst_inv = np.array([
        [(dst_w - 1) / 2, 0, (dst_w - 1) / 2],
        [0, (dst_h - 1) / 2, (dst_h - 1) / 2],
        [0, 0, 1]
    ])

    # src_pixel [0, src_w-1] -> src_norm [-1,1]
    N_src = np.array([
        [2 / (src_w - 1), 0, -1],
        [0, 2 / (src_h - 1), -1],
        [0, 0, 1]
    ...
    # 3. 组合: theta = N_src @ M_inv @ N_dst_inv
    theta_3x3 = N_src @ M_inv @ N_dst_inv
    theta = theta_3x3[:2, :]  # 取2x3

    # 转换为PyTorch tensor
    theta_tensor = torch.from_numpy(theta).float().unsqueeze(0)  # (1, 2, 3)

    return theta_tensor

# 1. 定义补齐函数
def to_homogeneous(theta):
    # theta shape: (B, 2, 3)
    B = theta.shape[0]
    # 创建底行为 [0, 0, 1] 的 3x3 矩阵
    res = torch.eye(3, device=theta.device, dtype=theta.dtype).repeat(B, 1, 1)
    res[:, :2, :] = theta
    return res
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        cls = labels["cls"]
        hierarchy = labels['hierarchy_names'] #luoru
        ...
        # ## ===================新增: luoru  单独的python theta 变换==============
        # theta_a = instances.affine
        # if isinstance(theta_a, np.ndarray):
        #     theta_a = torch.from_numpy(theta_a).float()
        # else:
        #     theta_a = theta_a.float()
        # h, w = img.shape[:2]
        # theta_rp = opencv_M_to_pytorch_theta(M, (w, h),(w, h))
        # # 完整的变换:
        # # 转换矩阵: 注意如果affine_matrix是template_b->img_a的，那么组合后是template_b->img_a_enhanced
        # theta_rp_h = to_homogeneous(theta_rp)  # 变成 (1, 3, 3)
        # theta_a_h = to_homogeneous(theta_a)  # 变成 (1, 3, 3)
        # #  矩阵乘法叠加 逻辑：先应用初始对齐 a，再应用增强 rp
        # theta_combined_h = torch.matmul(theta_a_h, theta_rp_h)
        # #  裁切回 (1, 2, 3) 给 grid_sample 使用
        # combined_affine_matrix = theta_combined_h[:, :2, :].numpy()
        ## =================================
        ...
        labels["cls"] = cls[i]
        labels['hierarchy_names'] = hierarchy[i] #luoru
        labels["img"] = img
...
def apply_flip_to_affine(theta, horizontal=False, vertical=False):
    """
    theta: [N, 2, 3] 原始仿射矩阵
    返回: 翻转后的仿射矩阵
    """
    ...
    # 构建翻转矩阵 [N, 2, 3]
    flip_mat = torch.eye(2, 3).unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    # 组合变换: theta @ flip_mat (注意：这里需要扩展为3x3做矩阵乘法)
    # 将 2x3 转为 3x3（齐次坐标）
    theta_3x3 = torch.cat([theta, torch.tensor([[[0, 0, 1]]]).repeat(batch_size, 1, 1)], dim=1)
    flip_3x3 = torch.cat([flip_mat, torch.tensor([[[0, 0, 1]]]).repeat(batch_size, 1, 1)], dim=1)

    # 新变换 = 原变换 @ 翻转矩阵
    new_theta_3x3 = torch.bmm(theta_3x3, flip_3x3)
    new_theta = new_theta_3x3[:, :2, :]  # 取回 2x3
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            img = np.flipud(img)
            instances.flipud(h)
            # # === 新增：同步翻转 affine 矩阵 === luoru
            # if isinstance(instances.affine, np.ndarray):
            #     theta_a = torch.from_numpy(instances.affine).float()
            # else:
            #     theta_a = instances.affine.float()
            # theta_a_new = apply_flip_to_affine(theta=theta_a, horizontal=False, vertical=True)
            # instances.affine = theta_a_new.numpy()
            # # =================================
        if self.direction == "horizontal" and random.random() < self.p:
            ...
            # else:
            #     theta_a = instances.affine.float()
            # theta_a_new = apply_flip_to_affine(theta=theta_a, horizontal=True, vertical=False)
            # instances.affine = theta_a_new.numpy()
            # =================================
            # For keypoints
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if channels == 6:
            ...
            h, w, c = img.shape
            pad_img = np.full((h + top + bottom, w + left + right, c), fill_value=0, dtype=img.dtype)
...
class ClassifyLetterBox:
    ...
    def __call__(self, im):
        ...
        # Create padded image
        if self.channels == 1:
            im_out = np.full((hs, ws), 0, dtype=im.dtype)
            im_out[top:top + h, left:left + w] = resized_im.squeeze()
        else:
            im_out = np.full((hs, ws, self.channels), 0, dtype=im.dtype)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        hierarchy = labels.pop('hierarchy_names') #luoru
        instances = labels.pop("instances")
        # labels_affines = instances.affine
        instances.convert_bbox(format=self.bbox_format)
        instances.denormalize(w, h)
        nl = len(instances)
        ...
        labels["img"] = self._format_img(img)
        labels["cls"] = torch.from_numpy(cls) if nl else torch.zeros(nl)
        labels['hierarchy_names'] = torch.from_numpy(hierarchy) if nl else torch.zeros(nl) #luoru
        labels["bboxes"] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
        # # luoru 新增
        # labels['affines'] = torch.from_numpy(labels_affines) if nl else torch.zeros((nl, 2, 3))  #luoru
        ...
        if self.return_obb:
            ...
            if len(instances.segments):
                points = torch.from_numpy(instances.segments)
                ordered_4pts = points.reshape(len(points), -1, 10)
            else:
                ordered_4pts = torch.zeros((0, 10))
            if isinstance(ordered_4pts, torch.Tensor):
                ordered_8pts = ordered_4pts.view(-1, 10)
                # 拼接成 (N, 13)
                labels["bboxes"] = torch.cat([std_bboxes, ordered_8pts], dim=-1)
            else:
                ordered_8pts = ordered_4pts.reshape(-1, 10)
                labels["bboxes"] = np.concatenate([std_bboxes, ordered_8pts], axis=-1)
                ####luoru ############
...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4,dtype=np.uint8):
        ...
        self.n = n
        self.dtype=dtype
        self.gray_value=0 if dtype==np.uint8 else 0*256
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            cls.append(labels['cls'])
            hierarchy.append(labels['hierarchy_names']) #luoru
            instances.append(labels['instances'])
        final_labels = {
            'im_file': mosaic_labels[0]['im_file'],
            'ori_shape': mosaic_labels[0]['ori_shape'],
            'resized_shape': (imgsz, imgsz),
            'cls': np.concatenate(cls, 0),
            'hierarchy_names': np.concatenate(hierarchy, 0), #luoru
            'instances': Instances.concatenate(instances, axis=0),
            'mosaic_border': self.border}  # final_labels
        final_labels['instances'].clip(imgsz, imgsz)
        good = final_labels['instances'].remove_zero_area_boxes()
        final_labels['cls'] = final_labels['cls'][good]
        final_labels['hierarchy_names'] = final_labels['hierarchy_names'][good] #gai
        return final_labels
