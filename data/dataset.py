...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data=None, task="detect",
                 names=None, hierarchy_names=None, **kwargs): #luoru
        """Initializes the YOLODataset with optional configurations for segments and keypoints."""
        self.use_segments = task == "segment"
        self.use_keypoints = task == "pose"
        self.use_obb = task == "obb"
        self.data = data
        self.names = names #luoru
        self.hierarchy_names = hierarchy_names  #Luoru
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg, hier in pbar: #luoru
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                hier_depth = len(hier) #luoru
                if im_file:
                    assert  len(lb[:, 0:1]) == len(hier)
                    x["labels"].append(
                        {
                            "im_file": im_file,
                            "shape": shape,
                            "cls": lb[:, 0:1],  # n, 1
                            "bboxes": lb[:, 1:],  # n, 4
                            # "affine": affine_data,  # 新增：存储放射变换矩阵 (N, 2, 3) 或 None
                            "segments": segments,
                            "keypoints": keypoint,
                            "normalized": True,
                            "bbox_format": "xywh",
                            "hierarchy_names" : hier, #luoru
                        }
                    )
                if msg:
                    msgs.append(msg)
    ...
    def get_labels(self):
        ...
        # 新增：重新计算放射变换文件路径（因为从缓存加载时可能未初始化） #luoru
        # self.affine_files = img2affine_paths(self.im_files)

        cache_path = Path(self.label_files[0]).parent.with_suffix(".cache")
        try:
            cache, exists = load_dataset_cache_file(cache_path), True  # attempt to load a *.cache file
            assert cache["version"] == DATASET_CACHE_VERSION  # matches current version
            assert cache["hash"] == get_hash(self.label_files + self.im_files )  # identical hash  luoru
    ...
    def update_labels_info(self, label):
        ...
        # segment_resamples = 100 if self.use_obb else 1000 #ori
        segment_resamples = 1 if self.use_obb else 1000  #luoru
        ...
        # 新增：提取放射变换矩阵 #luoru
        # affine_matrices = label.pop("affine", None)
        # 创建 Instances 对象，新增 affine 参数
        ...
        #     bbox_format=bbox_format,
        #     normalized=normalized,
        #     affine=affine_matrices  # 新增
        # )

        # 新增：提取放射变换矩阵 #luoru
        # 添加 affines 到 label 以便 collate_fn 处理
        # label["affines"] = affine_matrices
        return label
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            ###新增处理affine  luoru
            # if k == 'affines': #affine is the list of [N,2,3]
            #     value = torch.cat(value, 0)
