...
def verify_image_label(args):
        ...
        im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim = args  #ori
        # im_file, lb_file, affine_file, prefix, keypoint, num_cls, nkpt, ndim  = args #luoru
    ...
    # try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                hier = np.array([x[:-8] for x in lb], dtype=np.float32) #luoru
                # print('read hier', hier)
                lb = np.array([x[-9:] for x in lb], dtype=np.float32) #luoru
        ...
        # # 新增：验证放射变换文件
        # affine_data = verify_affine_file(affine_file)
        # # 如果存在放射变换，检查数量是否与标签匹配
        # # if affine_data is not None and len(affine_data) != len(lb):
        # if len(affine_data) != len(lb):
        #     LOGGER.warning(
        #         f"{prefix}WARNING ⚠️ {im_file}: affine matrix count ({len(affine_data)}) "
        #         f"does not match label count ({len(lb)}), ignoring affine data"
        #     )

        # return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg  # ori
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, hier  # luoru
        # return im_file, lb, shape, segments, keypoints, affine_data, nm, nf, ne, nc, msg  # luoru
...
def img2affine_paths(img_paths):
    """
    根据图像路径生成对应的放射变换文件路径 (.affine)
    假设放射变换文件存储在 labels 目录下，与 .txt 标签文件同名但后缀为 .affine
    """
    # 使用与 img2label_paths 类似的逻辑，但改为 .affine 后缀
    sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}affine{os.sep}"
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]


def verify_affine_file(affine_file):
    """
    验证并读取放射变换文件

    文件格式（每行对应一个目标的3x2仿射变换矩阵，共6个值）:
    a11 a12 a13 a21 a22 a23

    返回:
        affine_matrices: numpy array of shape (N, 2, 3) 或 None（如果文件不存在）
    """
    ...
    with open(affine_file, 'r') as f:
        for line in f:
            ...
            if line:
                ...
                values = values[:6] #gai luoru
                if len(values) == 6:
                    # reshape to 2x3 matrix
                    matrix = np.array(values).reshape(2, 3)
                    affine_list.append(matrix)
                else:
                    LOGGER.warning(
                        f"Invalid affine matrix format in {affine_file}: expected 6 values, got {len(values)}")
    return np.stack(affine_list, axis=0)  # shape: (N, 2, 3)
