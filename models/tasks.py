...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if m.i == len(self.model)-1:  # 最后一层 luoru
                x.append(saved_image_features)
                x.append(x_input)
            x = m(x)  # luoru

            # if m.i in [4]:  # 保存指定层的特征luoru
            if m.i in [6]:  # 保存指定层的特征luoru
            # if m.i in [9]:  # 保存指定层的特征luoru
            # if m.i in [15]:  # 保存指定层的特征luoru
                saved_image_features.append(x.clone())  # 保存特征
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, hier = {}, verbose=True):  # model, input channels, number of classes #luoru
        ...
        self.hierarchy_names = hier #luoru
        # self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, hierarchy_names=self.hierarchy_names,
                                            verbose=verbose)  # model, savelist   luoru
        ...
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            if use_Hier_head:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))[0]])  # forward  luoru
            else:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # forward  ori
...
class OBBModel(DetectionModel):
    ...
    def __init__(self, cfg="yolo11n-obb.yaml", ch=3, nc=None, hier= {}, verbose=True): #luoru
        """Initialize YOLO OBB model with given config and parameters."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, hier= hier, verbose=verbose) #luoru
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    model = (ckpt.get("ema") or ckpt["model"]).to(device).float()  # FP32 model
    # model = (ckpt["model"]).to(device).float()  # FP32 model  #luoru
...
def parse_model(d, ch, hierarchy_names, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset(
        {  WaveletDownBlock,
            ...
            C2,
            C2f,
            C3k2, C3k2_SAM, C3k2_DeepDBB,C3k2_DBB,C3k2_WDBB,C2f_DeepDBB,C2f_WDBB,C2f_DBB,C3k_RDBB,C2f_RDBB,C3k2_RDBB,A2C2f,DSC3k2,
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules :
            ...
            if m in repeat_modules :
                ...
                n = 1
            if m in (C3k2,  C3k2_SAM):  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
...
#-------------------------------------------------- YOLOv13 end------------------------------------------------------
            ...
            args.append(hierarchy_names)  #luoru
