...
class Focus(nn.Module):
    ...
    def __init__(self, c1, c2_list , k=1, s=1,  num_layers=3, p=None, g=1, act=True):
        """Initializes Focus object with user defined channel, convolution, padding, group and activation values."""
        super().__init__()
        c_in =3
        self.c2_len = len(c2_list)
        # 初始化空的ModuleList
        self.m = nn.ModuleList()
        self.out_conv = nn.ModuleList()
        self.num_layers = num_layers
        #  #按4^i的规律生成（i从1到num_layers）
        for i in range(1, self.num_layers + 1):
            # 每层的输入通道 = c_in * 4^i，输出通道按需定义
            if i < num_layers:  # 中间层：输入=输出
                conv = Conv(c_in * 4 ** i, c_in * 4 ** i, k, s, p, g, act=act)
            else:
                conv =Conv(c_in * 4 ** i, 128, k, s, p, g, act=act)
            self.m.append(conv)
        for i in range(len(c2_list)):  # 最后一层：输出为c2
             self.out_conv.append(Conv(128, c2_list[i], k, s, p, g, act=act))
    def forward(self, x):
        """
        ...
        for i in range(self.num_layers ):
           ...
           x= self.m[i](x)
        return [self.out_conv[i](x)  for i in range(self.c2_len) ]
...
class SilenceChannel(nn.Module):
    def __init__(self,c_start, c_end, shape=False):
        super(SilenceChannel, self).__init__()
        self.c_start=c_start
        self.c_end = c_end
        self.shape =shape

        # dict_cls = ['KC-135', 'Helicopter', 'E-8', 'E-3', 'C-130', 'C-17', 'P-8', 'P-3',
        #             'C-5', 'F', 'B-1', 'B-52', 'KC-10']  # same index with data.yaml
        # img_list = []
        # for cls in dict_cls:
        ...
        #     img_list.append(img)
        # # 4. 将列表堆叠为一个 Numpy 数组，形状变为 [B, H, W, C]
