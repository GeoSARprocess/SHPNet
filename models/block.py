...
class Proto_v2(nn.Module):
    def __init__(self, c1, c_=64, c2=1):
        super().__init__()
        self.upsample = nn.ConvTranspose2d(c1, c_//2, 2, 2, 0, bias=True)  # nn.Upsample(scale_factor=2, mode='nearest')
        self.upsample1 = nn.ConvTranspose2d(c_//2, c_ //4, 2, 2, 0, bias=True)  # nn.Up
        self.upsample2 = nn.ConvTranspose2d(c_//4, c_ //4, 2, 2, 0, bias=True)  # nn.Up
        ...
        self.final_conv = nn.Conv2d(c_ //2, c2, kernel_size=1)

    def forward(self, x):
        x = self.up_conv(self.upsample(x))
        # x = self.up_conv1(self.upsample1(x))
        # x = self.upsample2(x)
        x_out2  = self.final_conv(x)
        return x_out2
...
class C2f_SAM(nn.Module):
    ...
    def forward(self, x):
        ...
        y.extend(m(y[-1]) for m in self.m)
        # return self.cv2(torch.cat(y, 1)) #ori
        # print('forward')
        return  self.Simam_module(self.cv2(torch.cat(y, 1)))
