...
class Instances:
    ...
    def __init__(self, bboxes, segments=None, keypoints=None, bbox_format="xywh", normalized=True) -> None:
        ...
        # if affine is None:
        #     print(111)
            # default_2x3 = torch.tensor([[1., 0., 0.], [0., 1., 0.]], dtype=torch.float32)
            # affine = default_2x3.unsqueeze(0).repeat(bboxes.shape[0], 1, 1)
        # self.affine =  affine

    # 新增：获取放射变换的方法 #Luoru
    # def get_affine(self):
    #     """返回放射变换矩阵 (N, 2, 3) 或 None"""
    #     return self.affine
    ...
    def concatenate(cls, instances_list: List["Instances"], axis=0) -> "Instances":
        ...
        # # 拼接 affine luoru
        # affines_list = [ins.affine for ins in instances_list]
        # if all(a is not None for a in affines_list):
        #     cat_affine = torch.cat(affines_list, axis=axis)
        # else:
        #     cat_affine = None
        # # 拼接 affine luoru
        ...
        # return cls(cat_boxes, cat_segments, cat_keypoints, bbox_format, normalized) #ori
        # 创建新的 Instances，传入 affine luoru
