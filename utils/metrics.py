...
class Metric(SimpleClass):
    ...
    def mean_results(self): #平均结果 zcdmw
        """Mean of results, return mp, mr, map50, map."""
        return [self.mp, self.mr, self.map50, self.map, self.mf1]

    def class_result(self, i):#zcdmw
    ...
    def fitness(self):#zcdmw
        """Model fitness as a weighted combination of metrics.模型适应度作为指标的加权组合"""
...
class OBBMetrics(SimpleClass):
    ...
    def keys(self):
        """Returns a list of keys for accessing specific metrics."""
        return ["metrics/precision(B)", "metrics/recall(B)", "metrics/mAP50(B)", "metrics/mAP50-95(B)", 'metrics/F1'] #ZCDMW
